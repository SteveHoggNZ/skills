...
- **Consumers** (you want to *use* these skills): [install via git URL](#for-consumers). One command. Auto-updates at Claude Code startup.
- **Developers** (you want to *modify* these skills): [install from a local clone](#for-developers). Your on-disk edits become the installed state immediately.
