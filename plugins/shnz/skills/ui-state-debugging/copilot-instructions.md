# UI State Debugging (Copilot adapter)
...
- **Tool mapping**: VS Code's open-file for reference files; integrated terminal for the verification commands (DB, curl); apply-edit for diagnostic instrumentation.
...
2. **Open `.registry/<slug>.md`** if it exists — internalize the false-positive shapes and framework quirks before starting.
