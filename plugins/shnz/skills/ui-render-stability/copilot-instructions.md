# UI Render Stability (Copilot adapter)
...
- **Tool mapping**: VS Code's integrated terminal for ffmpeg; the built-in image preview for inspecting PNGs; apply-edit for the fix.
...
2. **Open `.registry/<slug>.md`** if it exists — internalize the known conditional-chrome quirks for this app.
